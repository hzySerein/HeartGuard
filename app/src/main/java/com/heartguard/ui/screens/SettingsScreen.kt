...
private fun UserProfileSettingsScreen(
    ...
    val profile = remember(context) { settingsManager.getUserProfile() }
        ...
        SaveSettingsButton(
            ...
            onClick = {
                ...
                if (safeNickname.isBlank()) {
                    ...
                    val savedProfile = settingsManager.getUserProfile()
...
private fun EmergencyContactsSettingsScreen(
    ...
    var contacts by remember(context) {
        mutableStateOf(settingsManager.getConfiguredEmergencyContacts())
    }

    fun updateContacts(updatedContacts: List<SettingsManager.EmergencyContact>) {
        contacts = updatedContacts
        settingsManager.saveEmergencyContacts(updatedContacts)
    }
    ...
    Box(
        ...
        SaveSettingsButton(
            modifier = Modifier.align(Alignment.BottomCenter),
            onClick = {
                settingsManager.saveEmergencyContacts(contacts)
...
private fun FontSizeSettingsScreen(
    ...
    var sizeLabel by remember(context) { mutableStateOf(settingsManager.getFontSize()) }
    ...
    Box(
        ...
        SaveSettingsButton(
            modifier = Modifier.align(Alignment.BottomCenter),
            onClick = {
                settingsManager.saveFontSize(sizeLabel)
...
private fun VoiceSettingsScreen(
    ...
    val context = LocalContext.current
    var volume by remember(context) { mutableStateOf(settingsManager.getVolume()) }
    var speed by remember(context) { mutableStateOf(settingsManager.getSpeed()) }
    var tone by remember(context) { mutableStateOf(settingsManager.getTone()) }
